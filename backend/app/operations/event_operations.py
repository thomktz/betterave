...
def get_association_events(asso: User, limit : int = None ) -> list[Event]:
    ...
    events = Event.query.filter_by(asso_id=asso.user_id).limit(limit).all() if limit is not None else Event.query.filter_by(asso_id=asso.user_id).all()
    return events
...
def get_user_future_events(user: User, limit : int = None) -> list[Event]:
    ...
    future_events = user.attended_events.filter(Event.date >= datetime.now().date()).limit(limit).all() if limit is not None else user.attended_events.filter(Event.date >= datetime.now().date()).all()
    return future_events
